...
- **Reuses the real site chrome** - `tokens.css`/`styles.css`, header/footer/ambient-grid (hand-copied like `pricing.html`/`donations.html`), `i18n.js`/`theme.js`/`nav.js` - via the same `env.ASSETS` binding, instead of a hand-rolled inline stylesheet. Needed a **second** `wrangler.toml` route, `downloads.tabularasa.cl/assets/*` - Cloudflare Routes are path-scoped, not hostname-wide, so without it those asset requests never reach the Worker at all (this broke once already - see the decision doc). Every asset path in `worker/index.js` must also be root-relative (`/assets/...`), a bare relative path would collide with the `/latest/*` route.
