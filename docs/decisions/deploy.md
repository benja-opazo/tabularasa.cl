...
a `main` script + **three** `routes` entries for the download-redirect
worker on `downloads.tabularasa.cl` (see `docs/decisions/download-redirect.md`
and `docs/decisions/seo.md`): `/latest/*` (the actual landing page),
`/assets/*` (so that page's CSS/JS/fonts, reused from the real site,
actually reach the Worker), and `/robots.txt` (a host-wide `Disallow: /` -
none of this is meant to be indexed). A Cloudflare Route only matches its
own path pattern, it doesn't hand this Worker the whole hostname, so each
needs its own entry. Do these **before** the first deploy that includes the
`routes` entries - a bad zone name fails at deploy time, not silently.
...
1. **Confirm the zone.** Cloudflare dashboard → your account → *Websites* -
   ...
   in a different zone, change `zone_name` on all three entries before
   deploying.
...
3. **Dry-run locally before deploying.** From the repo root:
   ...
   ```
   Bundles `worker/index.js` and validates `wrangler.toml` (including all
   three `routes` entries) without publishing anything - catches a bad
   `zone_name` or malformed route pattern here instead of mid-deploy.
...
5. **Confirm all three routes attached.** Dashboard → Workers & Pages →
   `tabularasa-cl` → Settings → Domains & Routes -
   `downloads.tabularasa.cl/latest/*`, `downloads.tabularasa.cl/assets/*`,
   and `downloads.tabularasa.cl/robots.txt` should all be listed as Routes,
   alongside the existing `tabularasa.cl` custom domain. Missing the
   `/assets/*` one is an easy mistake with a confusing symptom: the landing
   page itself loads (200), but loads completely unstyled with every script
   404ing, since its CSS/JS/font requests never reach the Worker at all - if
   that happens, check this list before anything else.
