...
site/donations.html               # Third page - "coming soon" stub all donate links point to. Same hand-duplicated shell. See donations.md.
...
hand rather than introducing a build step. **The flagged third page arrived**
(`donations.html`, see `docs/decisions/donations.md`) - re-examined the
shared-fragment-tooling question at that point and still called it not
worth it, since the new page is a minimal, likely-temporary placeholder, not
a reason to add build tooling to an otherwise build-free site. Revisit again
if a fourth, more substantial page shows up. Don't silently add more pages
without a matching decision entry.
